...
def venues():
  ...
  for area in areas:
    city = area.city
    state = area.state
    areaVenues = Venue.query.filter_by(city=city).all()
    venues = []
...
def show_venue(venue_id):
  ...
  who = Show.venue_id==venue_id
  shows_up = Show.query.filter(start<end).filter(who).all()
  upcoming_shows = []
  shows_past = Show.query.filter(start>end).filter(who).all()
  past_shows = []
  ...
  count_up = len(upcoming_shows)
  count_past = len(past_shows)
  data = ({
    "id": venues.id,
    "name": venues.name,
...
def create_venue_submission():
  ...
  venue = Venue(name=name, city=city, state=state, address=address, phone=phone, genres=genres,
   facebook_link=facebook_link, image_link=image_link, website=website, seeking_talent=seeking_talent, seeking_description=seeking_description)
...
def edit_artist_submission(artist_id):
  ...
  artists.website = request.form.get('website')
  artists.seeking_venue = True if 'seeking_venue' in request.form else False
...
def edit_venue_submission(venue_id):
  ...
  venues = Venue.query.get(venue_id)
  venues.name = request.form.get('name')
  venues.city = request.form.get('city')
  venues.state = request.form.get('state')
  venues.address = request.form.get('address')
  venues.phone = request.form.get('phone')
  venues.genres = request.form.getlist('genres')
  venues.facebook_link = request.form.get('facebook_link')
  venues.image_link = request.form.get('image_link')
  venues.website = request.form.get('website')
  venues.seeking_talent = True if 'seeking_talent' in request.form else False
  venues.seeking_description = request.form.get('seeking_description')
  # venue = Venue(name=name, city=city, state=state, address=address, phone=phone, genres=genres,
  #  facebook_link=facebook_link, image_link=image_link, website=website, seeking_talent=seeking_talent, seeking_description=seeking_description)
  db.session.commit()
...
def create_artist_submission():
  ...
  artist = Artist(name=name, city=city, state=state, phone=phone, genres=genres,
   facebook_link=facebook_link, image_link=image_link, website=website, seeking_venue=seeking_venue, seeking_description=seeking_description)
